...
class AIChatbotProvider with ChangeNotifier {
  ...
  void initializeWithData(Users user, List<Workout> workouts, List<Exercise> exercises) {
    _userName = user.fullName ?? 'User';
    _workouts = workouts;
    ...
    try {
      ...
      if (_workouts.isEmpty) {
        final aiMessage = Message(
          content: "I couldn't find any workouts in the database. Please try again later.",
          isUser: false,
        );
        _messages.add(aiMessage);
        addSuggestions();
      ...
      if (user.fitnessLevel != null && user.fitnessLevel!.isNotEmpty) {
        final matchingLevelWorkouts = _workouts.where(
          (w) => w.fitnessLevel.toLowerCase() == user.fitnessLevel!.toLowerCase()
        ).toList();
      ...
      if (user.goalType != null && user.goalType!.isNotEmpty && filteredWorkouts.length > 1) {
        final matchingGoalWorkouts = filteredWorkouts.where(
  ...
  Future<void> recommendExercise(Users user) async {
    ...
    try {
      ...
      if (_exercises.isEmpty) {
        final aiMessage = Message(
          content: "I couldn't find any exercises in the database. Please try again later.",
          isUser: false,
        );
        _messages.add(aiMessage);
        addSuggestions();
